      ...
      _export("SkinController", SkinController = (_dec = ccclass('SkinController'), _dec2 = executeInEditMode(true), _dec3 = property([SpriteFrame]), _dec4 = property({
        ...
        set currentSkinIndex(value) {
          ...
          if (intIdx < 0) {
            ...
            var safeIdx = Math.min(intIdx, this.frames.length - 1);

            if (this._currSkinIdx !== safeIdx) {
              this._currSkinIdx = safeIdx;

              var _sprite = this.getComponent(Sprite);
        ...
        setBoneSkinIndex(index) {
          ...
          var sf = this.frames[index];

          if (sf) {
            sprite.enabled = true;
            sprite.spriteFrame = sf;
            var origW = sf.originalSize.width;
            var origH = sf.originalSize.height;
            var rectW = sf.rect.width;
            var rectH = sf.rect.height;
            var offsetX = sf.offset.x;
            var offsetY = sf.offset.y;
            var pivotOrigX = origW * this.baseAnchorX;
            var pivotOrigY = origH * this.baseAnchorY;
            var rectLeft = origW / 2 + offsetX - rectW / 2;
            var rectBottom = origH / 2 + offsetY - rectH / 2;
            var calcX = rectW > 0 ? (pivotOrigX - rectLeft) / rectW : 0.5;
            var calcY = rectH > 0 ? (pivotOrigY - rectBottom) / rectH : 0.5;
            var finalAnchorX = Math.max(0.0, Math.min(1.0, calcX));
            var finalAnchorY = Math.max(0.0, Math.min(1.0, calcY));
