      ...
      _export("BoneFollower", BoneFollower = (_dec = ccclass('BoneFollower'), _dec2 = executeInEditMode(true), _dec3 = property({
        ...
        lateUpdate() {
          if (this.targetBone && this.targetBone.isValid && this.node.parent) {
            // 1. 타겟의 월드 매트릭스 획득
            var targetWorldMat = this.targetBone.worldMatrix; // 2. 내 부모(RenderRoot)의 월드 매트릭스의 역행렬 구하기

            var parentWorldMat = this.node.parent.worldMatrix;
            ...
            var scaleX = Math.sqrt(m00 * m00 + m01 * m01);
            var scaleY = Math.sqrt(m04 * m04 + m05 * m05); // 💡 외적(Cross Product)을 통한 진짜 부호 판별!
            // 2D 공간 행렬식이 음수면 X축이 거울처럼 뒤집힌 것(음수 스케일)입니다.

            var cross = m00 * m05 - m04 * m01;
            ...
            var rotDeg = rotRad * 180 / Math.PI; // 6. 🚨 엔진의 setWorld... API를 피해서 로컬 속성에 직접 꽂아 넣습니다!
